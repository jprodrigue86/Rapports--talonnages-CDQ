(function () {
  ...
  function apply() {
    ...
    const top = 16 + i * 0.16, bottom = 16 + i * 0.16, label = 9 + t * 0.03;
    const text = 12 + t * 0.04;
    const height = Math.ceil(Math.max(48 * density, bottom + label * 1.2 + 14 * density, 44));
    const navHeight = 'calc(' + height + 'px + var(--cdq-safe-bottom))';
    root.style.setProperty('--cdq-nav-height', navHeight);
    Object.entries({bottom, top, quick: top, file: 20 + i * 0.16, action: 12 + i * 0.1})
    ...
    const buttonHeight = Math.ceil(Math.max(44, 54 * density, top + label * 1.2 + 14));
    each('#cdqTopActionsV2204 > .cdq-top-action, .quick-button', {'min-height': px(buttonHeight),
      ...
      'padding': px(5 * density) + ' 3px', 'gap': px(3 * density), 'white-space': 'normal'});
    each('.quick-button', {'min-height': px(Math.max(60, 66 * density, top + label * 2.4 + 16))});
    ...
    each('.client-info-name', {'font-size': px(text + 1)});
    each('.client-stats', {'font-size': px(10 + t * 0.03), 'white-space': 'normal'});
    ...
    each('.company-reset-button', {'height': px(Math.max(44, 48 * density))});
