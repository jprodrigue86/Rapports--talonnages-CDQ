(function () {
  ...
  function apply() {
    ...
    const density = 0.8 + g * 0.004;
    const top = 14 + i * 0.12, bottom = 12 + i * 0.12, label = 8.5 + t * 0.03;
    const text = 13.5 + t * 0.04, action = 12 + i * 0.08;
    const height = Math.ceil(Math.max(44 * density, bottom + label * 1.2 + 12 * density, 44));
    const navHeight = 'calc(' + height + 'px + var(--cdq-safe-bottom))';
    root.style.setProperty('--cdq-nav-height', navHeight);
    Object.entries({bottom, top, quick: top, file: 20 + i * 0.16, action})
    ...
    if (nav) {
      ...
      set(nav, {'height': navHeight, 'min-height': navHeight, 'max-height': navHeight,
        'padding': '4px 2px calc(4px + var(--cdq-safe-bottom))', 'gap': '0', 'z-index': '1000',
        'grid-template-columns': 'repeat(' + count + ',minmax(0,1fr))', 'grid-template-rows': 'minmax(0,1fr)', 'align-items': 'stretch'});
    ...
    // The existing folder glyph contains two characters (folder + plus), not one icon.
    const folderIcon = document.getElementById('folderActionIcon');
    if (folderIcon && folderIcon.textContent.trim() === '📁+') {
      set(folderIcon, {'width': px(bottom * 1.5), 'min-width': px(bottom * 1.5), 'max-width': px(bottom * 1.5), 'font-size': px(bottom * 0.8)});
    }
    each('.bottom-nav > .bottom-nav-item small', {'font-size': px(Math.min(label, (window.innerWidth - 24) / 34)), 'line-height': '1.15',
      'padding': '0', 'margin': '0', 'max-width': '100%', 'white-space': 'normal', 'overflow-wrap': 'anywhere', 'text-overflow': 'clip'});
    const buttonHeight = Math.ceil(Math.max(44, 44 * density, top + label * 1.2 + 10));
    each('#cdqTopActionsV2204 > .cdq-top-action, .quick-button', {'min-height': px(buttonHeight),
      ...
      'padding': px(3 * density) + ' 2px', 'gap': px(2 * density), 'white-space': 'normal'});
    each('.quick-button', {'min-height': px(Math.max(44, 52 * density, top + label * 2.4 + 8))});
    ...
    each('#cdqTopActionsV2204 > .cdq-top-action > span:last-child, .quick-name', {'font-size': px(label),
      'line-height': '1.2', 'white-space': 'normal', 'overflow-wrap': 'anywhere', 'word-break': 'normal',
      'min-width': '0', 'width': '100%', 'max-width': '100%', 'text-overflow': 'clip'});
    icon('.file-icon, .file-icon svg', 20 + i * 0.16);
    ...
    each('.file-name, .file-name-text, .folder-name, .folder-name-text', {'font-size': px(text), 'line-height': '1.25', 'white-space': 'normal', 'overflow-wrap': 'anywhere'});
    each('.file-date', {'font-size': px(10 + t * 0.03), 'line-height': '1.25'});
    each('.client-info-name', {'font-size': px(12 + t * 0.03)});
    each('.client-stats', {'font-size': px(9 + t * 0.03), 'white-space': 'normal', 'margin-top': '3px'});
    each('.file-row, .folder-header', {'min-height': px(Math.max(44, 48 * density)), 'padding-top': px(5 * density), 'padding-bottom': px(5 * density)});
    each('.company-button, .company-reset-button', {'min-height': px(Math.max(44, 44 * density)), 'padding-top': '4px', 'padding-bottom': '4px'});
    each('.company-button', {'font-size': px(11.5 + t * 0.03)});
    each('.company-reset-button', {'height': px(Math.max(44, 44 * density)), 'font-size': px(top + 2)});
    icon('.company-reset-button svg', top + 2);
    each('.app-header', {'height': px(42 * density), 'min-height': '0', 'padding': '0', 'margin': '2px 0 6px'});
    each('.header-metal-banner', {'height': '100%', 'width': '100%', 'object-fit': 'contain'});
    each('.cdq-swipe-overlay', {'gap': '4px', 'padding': '2px 6px'});
    each('.cdq-swipe-action', {'height': px(Math.max(44, 44 * density)), 'min-height': '44px',
      'min-width': '44px', 'max-width': '100%', 'padding': '2px 6px', 'border-radius': '8px',
    ...
    each('.cdq-swipe-action.note,.cdq-swipe-action.photo,.cdq-swipe-action.favorite,.cdq-swipe-action.send,.cdq-swipe-action.protect',
      {'font-size': px(10 + t * 0.03), 'min-width': '44px'});
    window.cdqMobileDimensions = {density, top, bottom, label, navHeight: height, buttonHeight, text, action};
  }
  let queued = false;
  function schedule() {
    if (queued) return;
