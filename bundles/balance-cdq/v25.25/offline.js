...
const cdqOffline24 = (() => {
  ...
  function paintDots(){
    ...
    document.querySelectorAll('.file-row,.cdq23-drive-row,[data-file-id],.pc17-files tbody tr').forEach(row=>{
      ...
      if(ids.has(String(id))&&!dot){dot=el('span','', 'cdq24-offline-dot');dot.title='Disponible hors ligne';dot.setAttribute('aria-label','Disponible hors ligne');host.prepend(dot);}
