...
class StartupTicketStore(private val context: Context) {
    data class Ticket(val email: String, val expiresAt: Long)

    companion object {
        private const val KEY_ALIAS = "balance_cdq_startup_ticket_v2539"
        private const val PREFS = "balance_cdq_startup_ticket_v2539"
        private const val VALUE = "ticket"
        private const val TRANSFORMATION = "AES/GCM/NoPadding"
        private const val IV_BYTES = 12

        // A normal work shift remains fast, but every actual app reopen still
        // requires Android biometrics. Remote access is never granted by this ticket.
    ...
    private fun decrypt(now: Long): JSONObject? {
        return try {
    ...
    fun peek(now: Long = System.currentTimeMillis()): Ticket? {
        val payload = decrypt(now) ?: return null
        val email = normalizeEmail(payload.optString("email"))
        if (email.isBlank()) return clearAndNull()
        return Ticket(email, payload.optLong("expiresAt"))
    }
    ...
    fun read(
        ...
        val payload = decrypt(now) ?: return null
        if (normalizeEmail(payload.optString("email")) != normalized) return null

        val storedHash = try {
            Base64.decode(payload.optString("tokenHash"), Base64.DEFAULT)
        } catch (_: Exception) {
            return clearAndNull()
        }
        if (!MessageDigest.isEqual(storedHash, tokenDigest(token))) return null

        return Ticket(normalized, payload.optLong("expiresAt"))
    }
    ...
    private fun clearAndNull(): Nothing? {
        clear()
        return null
    }
}
