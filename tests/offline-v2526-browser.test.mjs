...
const read=p=>fs.readFileSync(p,'utf8'),dir='bundles/balance-cdq/v25.26';
const full=process.env.CDQ_V2526_SELECTOR?read(process.env.CDQ_V2526_SELECTOR):'';
const styles=(full?[...full.matchAll(/<style\b[^>]*>[\s\S]*?<\/style>/gi)].map(m=>m[0]).join(''):'<style>'+read('bundles/balance-cdq/v25.16/mobile-layout.css')+read('bundles/balance-cdq/v25.24/interface.css')+read('bundles/balance-cdq/v25.25/interface.css')+'</style>');
const setup=`
...
const cdqDefaultGoogleAccountV2299=()=> 'google@example.invalid';const cdqOpenSheetV2526=id=>{window.sheetOpened=id;};
...
<script>${setup}</script><script>${read('bundles/balance-cdq/v25.16/mobile-layout.js')}</script><script>${read(dir+'/offline.js')}</script><script>${read('bundles/balance-cdq/v25.24/company.js')}</script><script>document.getElementById('cdqV19OfflineStatus').onclick=()=>cdqOffline24.open().catch(afficherErreur);</script></body></html>`;
...
import {createOfflineTemplates} from '/offline-templates-v2526.mjs';window.sent=[];window.opened=[];window.sheetOpened=[];window.unlocked=0;
window.controller=createOfflineTemplates({send:data=>{sent.push(data);document.getElementById('selector').contentWindow.postMessage(data,location.origin)},unlock:async()=>{unlocked++},openPdf:data=>{opened.push(data)},openSheetFile:data=>{sheetOpened.push(data)}});
...
try{
 ...
 assert.equal(await frame.$$eval('.cdq24-checklist input',els=>els.length),3);
 ...
 // Main-list opening and editing use the shell's durable copy, never a second independent queue.
 const record=await frame.evaluate(async()=>{window.canonical=await cdqOffline24.load('pdf_one');return {id:canonical.id,version:canonical.offlineVersion};});assert.equal(record.id,'pdf_one');
 await frame.evaluate(async()=>{await cdqOffline24.save(canonical,new Blob(['%PDF-1.7 main edits'],{type:'application/pdf'}),'save-main-123456789');});
 await page.waitForFunction(()=>sent.some(m=>m.type==='CDQ_OFFLINE_SAVE'));
 const mainUpload=await page.evaluate(()=>sent.find(m=>m.type==='CDQ_OFFLINE_SAVE'));assert.match(mainUpload.uploadId,/^[a-zA-Z0-9_-]{8,100}$/);
 await frame.evaluate(async()=>{window.removeError='';try{await cdqOffline24.remove('pdf_one')}catch(e){removeError=e.message}});assert.match(await frame.evaluate(()=>removeError),/modifications/);
 await page.evaluate(async()=>{const m=sent.find(m=>m.type==='CDQ_OFFLINE_SAVE');await controller.handle({type:'CDQ_OFFLINE_SAVE_RESULT',requestId:m.requestId,uploadId:m.uploadId,ok:true,id:m.driveId,revision:'rev2'});sent.length=0;});
 await frame.waitForFunction(async()=>{const d=await cdqOffline24.load('pdf_one');return d.revision==='rev2'});
 ...
 // Selecting a Sheet persists a pending shortcut, not a fabricated PDF or ready flag.
 await frame.evaluate(async()=>{await cdqOffline24.open();document.querySelector('.cdq24-checklist input[value="sheet_one"]').checked=true;[...document.querySelectorAll('#cdqDocumentPrepare button')].find(b=>b.textContent==='Préparer les fichiers cochés').click();});
 await frame.waitForFunction(()=>cdqOffline24.job&&!cdqOffline24.job.running);
 await frame.waitForFunction(()=>document.querySelector('[data-stored24]').textContent.includes('À activer dans Google Sheets'));
 assert.equal(await frame.evaluate(()=>calls.some(c=>c[0]==='obtenirPdfLecteurCDQV2520'&&c[1]==='sheet_one')),false);
 await frame.evaluate(()=>[...document.querySelectorAll('[data-stored24] button')].find(b=>b.textContent==='Feuille').click());assert.equal(await frame.evaluate(()=>sheetOpened),'sheet_one');
 await frame.evaluate(()=>[...document.querySelectorAll('[data-stored24] button')].find(b=>b.textContent==='J’ai activé le hors connexion').click());
 await frame.waitForFunction(()=>document.querySelector('[data-stored24]').textContent.includes('confirmé par vous'));
 ...
 await page.evaluate(async()=>{Object.defineProperty(navigator,'onLine',{configurable:true,value:false});await controller.refresh();});assert.equal(await page.$eval('#cdq-offline-launch',e=>getComputedStyle(e).display),'none');await page.waitForFunction(()=>[...document.querySelectorAll('button')].some(b=>b.textContent==='Déverrouiller avec la sécurité de l’appareil'));
 ...
 assert.equal(await page.evaluate(()=>opened[0].blob.text()),'%PDF-1.7 main edits');assert.equal(await page.evaluate(()=>unlocked),1);
 await page.waitForFunction(()=>document.body.innerText.includes('Feuille — Hors connexion confirmé par vous'));
 await page.evaluate(()=>[...document.querySelectorAll('section button')].find(b=>b.textContent==='Ouvrir dans Google Sheets').click());
 assert.equal(await page.evaluate(()=>sheetOpened[0].id),'sheet_one');assert.equal(await page.evaluate(()=>sheetOpened[0].email),'google@example.invalid');
 ...
 await page.evaluate(async()=>{Object.defineProperty(navigator,'onLine',{configurable:true,value:true});await controller.handle({type:'CDQ_OFFLINE_SESSION',email:'a@example.invalid',canWrite:true,protocol:38});});
 const save=await page.evaluate(()=>sent.find(m=>m.type==='CDQ_OFFLINE_SAVE'));assert.equal(save.driveId,'pdf_one');assert.equal(save.revision,'rev2');
 ...
 await frame.waitForFunction(async()=>!await cdqV19GetRecord('document-pdf','pdf_one'));await frame.evaluate(()=>cdqOffline24.refresh());assert.equal(await frame.evaluate(async()=>!!await cdqV19GetRecord('document-pdf','pdf_one')),false);
 ...
 await page.screenshot({path:'/tmp/cdq-v2526-offline.png',fullPage:true});
 assert.deepEqual(errors,[]);console.log('PASS V25.26: selective preparation, progress, reopen, cancel, durable restart, unlock, edit/sync/removal, company dots, stable navigation.');
