...
try{
 ...
 await page.click('.copy-confirm');await page.waitForFunction(()=>opened.length===1);
 ...
 await page.click('.copy-confirm');await page.waitForFunction(()=>opened.length===1);
 if(process.env.CDQ_TEST_READER==='1')await page.waitForFunction(()=>document.querySelector('#legacy-pdf-reader')?.contentDocument?.querySelectorAll('.annotationLayer input').length>100,{timeout:20000});
 assert.equal(await page.evaluate(()=>rpc.length),0);assert.deepEqual(await page.evaluate(()=>errors),[]);assert.deepEqual(errors,[]);
 console.log('PASS: actual copy handler, offline first-use, no Drive/RPC, embedded bytes, reader handoff, durable IDB after reload.');
