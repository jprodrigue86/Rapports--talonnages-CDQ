...
const mime={html:'text/html',js:'text/javascript',mjs:'text/javascript',json:'application/json',webmanifest:'application/manifest+json',css:'text/css',png:'image/png',webp:'image/webp',jpg:'image/jpeg',svg:'image/svg+xml',wasm:'application/wasm',pdf:'application/pdf',ttf:'font/ttf'};
...
async function ui(engine,name){
 ...
 try{
  const context=await browser.newContext({...devices['iPhone 13'],serviceWorkers:'block'}),errors=[];
  ...
  await context.route('**/*',async route=>{
   ...
   if(url.startsWith('https://accounts.google.com/gsi/client'))return route.fulfill({contentType:'text/javascript',body:'window.google=window.google||{};window.google.accounts={id:{initialize(){},renderButton(){},prompt(){}}};'});
   if(url.startsWith('https://script.google.com/')&&url.includes('cdq_native_bridge=1')){
    ...
    return route.fulfill({contentType:'text/html',body:`<iframe src="https://fixture-script.googleusercontent.com/bridge?channel=${channel}"></iframe>`});
   }
   if(url.startsWith('https://fixture-script.googleusercontent.com/bridge')){
    const channel=new URL(url).searchParams.get('channel');
    const fixture=`window.google={script:{run:new Proxy({},{get(_target,name){
     ...
     return (...args)=>{const done=window.success;
    ...
    return route.fulfill({contentType:'text/html',body:`<script>const CDQ_EMBEDDED_CHANNEL=${JSON.stringify(channel)};${fixture}\n${bridge}</script>`});
   }
   return route.abort();
  });
  ...
  await page.waitForFunction(()=>document.querySelector('#app')?.contentDocument?.querySelector('#accessOverlay'),{timeout:45000});
  const selector=page.frames().find(f=>f.url()===app+'Selector.html');assert(selector);
...
async function updates(engine,name){
 ...
 const browser=await engine.launch();
 try{
  const context=await browser.newContext({...devices['iPhone 13']}),page=await context.newPage(),errors=[];
