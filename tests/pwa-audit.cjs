...
async function main(){
 ...
 await test('Une vraie nouvelle version PWA est détectée',async()=>{const h=pwa(),source=announce(h);h.c.fetch=async()=>({ok:true,json:async()=>({version:'2099.01.01.0000-v99.0'})});h.fire('message',{source,origin,data:{type:'CDQ_CHECK_UPDATE',build:'ancien'}});await tick();assert.equal(h.sent.at(-1).d.available,true);});
