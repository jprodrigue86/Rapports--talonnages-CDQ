...
function domHarness(){
 const timers=new Map(),listeners={},els=new Map(),sent=[];let seq=0;
 ...
 return {c,win,document,el,app,fire,timers,runTimers,sent};
}
...
async function main(){
 ...
 await test('Biométrie et Push reçus seulement du Selector identifié',()=>{const h=pwa(),source=announce(h);h.fire('message',{source,origin,data:{type:'CDQ_BIOMETRIC_REQUEST',email:'test@example.invalid'}});assert.equal(h.el('bio-setup').style.display,'flex');assert.equal(h.sent.at(-1).o,origin);h.fire('message',{source,origin,data:{type:'CDQ_PUSH_REQUEST'}});assert.equal(h.el('push-setup').style.display,'flex');h.el('push-cancel').listeners.click();assert.equal(h.sent.at(-1).d.type,'CDQ_PUSH_CANCELLED');});
 ...
 await test('Une vraie nouvelle version PWA est détectée',async()=>{const h=pwa(),source=announce(h);h.c.fetch=async()=>({ok:true,json:async()=>({version:'2026.09.15.0000-v21.24'})});h.fire('message',{source,origin,data:{type:'CDQ_CHECK_UPDATE',build:'ancien'}});await tick();assert.equal(h.sent.at(-1).d.available,true);});
 await test('Service worker conserve les caches des autres applications',async()=>{const handlers={},deleted=[],waiting=[];const c=vm.createContext({URL,Response,console,importScripts(){},self:{registration:{scope:'https://jprodrigue86.github.io/Rapports--talonnages-CDQ/'},clients:{claim:async()=>{}},addEventListener:(n,f)=>handlers[n]=f},caches:{keys:async()=>['cdq-installable-old','techbench-cache','cdq-offline-data','cdq-installable-v21-23'],delete:async k=>deleted.push(k)}});vm.runInContext(read('sw.js'),c);handlers.activate({waitUntil:p=>waiting.push(p)});await Promise.all(waiting);assert.deepEqual(deleted,['cdq-installable-old']);});
