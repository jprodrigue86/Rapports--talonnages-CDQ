...
export const manifest=JSON.parse(read('bundles/balance-cdq/v25.18/manifest.json'));
export function patch(source,file,m=manifest){return m.patches.filter(p=>p.file===file).reduce((s,p)=>applyPatch(s,p),source)}
export function currentSource(){let source=read(process.env.CDQ_SELECTOR_SOURCE);for(const v of ['25.15','25.16','25.17'])source=patch(source,'Selector.html',JSON.parse(read(`bundles/balance-cdq/v${v}/manifest.json`)));return source}
...
window.calls=[];window.completed=[];window.opened=[];window.cdqRafraichirTechniciensRapports=()=>{};window.modifierFichier=id=>opened.push(id);
...
window.rpcIcons={};window.delays={obtenirRapportsRecentsPC:1200};window.rpcFailures={};
window.demanderCopie=id=>calls.push({name:'create-report',args:[id,compagnieSelectionnee]});
window.cdqPcSettingsV2518={save:()=>calls.push({name:'save-prefs'}),check:()=>calls.push({name:'check-update'}),diagnostic:()=>calls.push({name:'diagnostic'})};
window.cdqGoogleDefaultAccountV2294=()=>localStorage.getItem('cdqDefaultGoogleAccountV2294')||'';
window.cdqSetGoogleDefaultAccountV2294=email=>{if(email&&!/^[^@]+@[^@]+\.[^@]+$/.test(email))throw Error('Adresse invalide');localStorage.setItem('cdqDefaultGoogleAccountV2294',email)};
...
window.rpcResult=(name,args)=>{
 ...
 if(name==='obtenirRapportsRecentsPC')return demoFiles.slice(0,15).map((f,i)=>i===0?{...f,statut:'inconnu'}:f);
...
window.cdqApiRun=()=>{let ok=()=>{},fail=()=>{};const proxy=new Proxy({},{get:(_,name)=>name==='withSuccessHandler'?f=>{ok=f;return proxy}:name==='withFailureHandler'?f=>{fail=f;return proxy}:(...args)=>{calls.push({name,args});setTimeout(()=>{try{if(rpcFailures[name])throw Error(rpcFailures[name]);completed.push(name);ok(rpcResult(name,args))}catch(e){fail(e)}},delays[name]||12)}});return proxy};
...
export function fixture(){
 ...
 if(process.env.CDQ_SELECTOR_SOURCE){
  ...
  const iconPatch=JSON.parse(read('bundles/balance-cdq/v25.17/manifest.json')).patches.filter(p=>p.search?.startsWith('window.cdqIconThemes')||p.search?.includes('function accept('));
  let icons=read('bundles/balance-cdq/v25.15/icon-themes.js');for(const p of iconPatch)icons=applyPatch(icons,p);
  styles='<style>'+read('bundles/balance-cdq/v25.15/icon-themes.css')+'</style><style>'+read('bundles/balance-cdq/v25.18/desktop.css')+'</style>';
  scripts='<script>'+read('bundles/balance-cdq/v25.18/desktop.js')+'</script><script>'+icons+'</script><script>'+mockCore+'</script>';
 }
