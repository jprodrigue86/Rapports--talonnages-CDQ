...
const read=p=>fs.readFileSync(p,'utf8'),dir='bundles/balance-cdq/v25.25';
const full=process.env.CDQ_V2525_SELECTOR?read(process.env.CDQ_V2525_SELECTOR):'';
const styles=(full?[...full.matchAll(/<style\b[^>]*>[\s\S]*?<\/style>/gi)].map(m=>m[0]).join(''):'<style>'+read('bundles/balance-cdq/v25.16/mobile-layout.css')+read('bundles/balance-cdq/v25.24/interface.css')+read(dir+'/interface.css')+'</style>');
const setup=`
...
import {createOfflineTemplates} from '/offline-templates-v2525.mjs';window.sent=[];window.opened=[];window.unlocked=0;
...
try{
 ...
 await page.screenshot({path:'/tmp/cdq-v2525-offline.png',fullPage:true});
 assert.deepEqual(errors,[]);console.log('PASS V25.25: selective preparation, progress, reopen, cancel, durable restart, unlock, edit/sync/removal, company dots, stable navigation.');
