...
try{
 for(const mobile of [false,true]){
  ...
  const input=async(value)=>{await f.$eval('input[name="client_nom"]',(e,v)=>{e.focus();e.value=v;e.dispatchEvent(new Event('input',{bubbles:true}));e.dispatchEvent(new Event('change',{bubbles:true}));const done=document.getElementById('doneFields');if(done&&!done.hidden)done.click();else e.blur();},value);await f.waitForFunction(()=>!document.body.classList.contains('cdq-keyboard-field'),{timeout:1500});};
