...
test('Installation complète et cache hors ligne du moteur et du modèle pour PC et téléphone',()=>{
 for(const [path,relative] of [['index.html','./'],['pc/index.html','../']])assert.ok(read(path).includes("import('"+relative+"offline-templates-v2519.mjs')"));
 for(const path of ['sw.js','pc/sw.js'])for(const file of ['offline-templates-v2519.mjs','floor-template-v2519.mjs'])assert.ok(read(path).includes(file));
