...
try{
 ...
 assert.equal(await page.$eval('#loading .box',e=>getComputedStyle(e).backgroundColor),'rgba(0, 0, 0, 0)');assert.equal(await page.$eval('#loading .loading-title',e=>getComputedStyle(e).display),'none');await page.screenshot({path:'/tmp/cdq-v2525-startup.png'});
 ...
 assert.equal(await page.$eval('#cdq-offline-launch',e=>getComputedStyle(e).display),'none');await page.waitForFunction(()=>[...document.querySelectorAll('section button')].some(b=>b.textContent==='Déverrouiller avec la sécurité de l’appareil'));
 ...
 assert.equal(await page.evaluate(()=>navigator.onLine),false);await page.screenshot({path:'/tmp/cdq-v2525-offline-restart-reader.png'});
 console.log('PASS V25.25: actual startup loader, service-worker offline restart after closing tab, native unlock bridge and actual PDF reader.');
