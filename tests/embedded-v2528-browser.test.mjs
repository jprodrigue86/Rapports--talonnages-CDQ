...
const prefix=base+'native/v25.49/';
...
try{
  ...
  await page.setUserAgent('Mozilla/5.0 (Linux; Android 16) AppleWebKit/537.36 Chrome/140.0 Mobile Safari/537.36 BalanceCDQAndroid/25.49 CDQSafeArea/1');
  ...
  await page.evaluateOnNewDocument(()=>{
    ...
    window.BalanceCDQNative={
      ...
      updateIdentity(){return JSON.stringify({versionName:'25.49',versionCode:2549});},
  ...
  page.on('request',async request=>{
    ...
    if(url.includes('/downloads/android-release-update.json'))return request.respond({status:200,contentType:'application/json',headers:{'Access-Control-Allow-Origin':'*','Cache-Control':'no-store'},body:JSON.stringify({versionName:'25.50',versionCode:2550,apkUrl:'https://example.invalid/Balance-CDQ-Android-25.50.apk',sha256:'deadbeef',channel:'stable'})});
    // The explicitly live version check is allowed; public/static UI downloads are not.
    if(url.includes('/bundles/balance-cdq/latest/manifest.json')||url.includes('/version.json'))return request.respond({status:200,contentType:'application/json',body:JSON.stringify({version:'V25.31',build:'2026.09.26-v25.49-reader-keyboard-space'})});
  ...
  // V25.49 returning startup: Android already owns the biometric prompt and
  ...
  assert.equal(updateUi.installed,'V25.49');
  assert.equal(updateUi.latest,'V25.50');
  assert.match(updateUi.state,/disponible/i);
  assert.match(updateUi.installText,/Installer V25\.50/);
  ...
  assert.match(navIcons.sprite.url,/native\/v25\.49\/bundles\/balance-cdq\/v25\.15\/icons-transparent\.webp/);
