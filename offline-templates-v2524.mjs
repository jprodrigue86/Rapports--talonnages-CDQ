...
export function createOfflineTemplates({send,unlock,openPdf,warmPdf,storage=nativeStorage}) {
  ...
  function notifyDocument(c){if(c?.kind==='prepared'&&!c.removed)send({type:'CDQ_OFFLINE_DOCUMENT_UPDATED',email:c.email,fileId:c.driveId,blob:c.blob,revision:c.revision,name:c.name,clientId:c.destination.clientId,clientName:c.destination.name});}
  ...
  async function refresh() {
    ...
    launch.hidden=!!session||!copiesHere;
    ...
    for(const c of copies.filter(c=>c.kind!=='prepared')){
  ...
  async function saveCopy(id,email,blob,readerRequestId) {
    ...
    return serial(async()=>{
      ...
      if(c.readerRequestId!==readerRequestId){
        const updated={...c,blob,readerRequestId,editVersion:(c.editVersion||0)+1,uploadId:id+'_e_'+crypto.randomUUID(),status:'pending',error:''};
  ...
  async function handleNow(data,stamp) {
    ...
    if(['CDQ_OFFLINE_DOCUMENT_STORE','CDQ_OFFLINE_DOCUMENT_REMOVE','CDQ_OFFLINE_DOCUMENT_LIST'].includes(data.type)){
      ...
      try{
        if(data.type==='CDQ_OFFLINE_DOCUMENT_LIST'){
          ...
          if(stamp===epoch)reply({ok:true,removed});return;
        }
        ...
        if(data.type==='CDQ_OFFLINE_DOCUMENT_REMOVE'){
          ...
          await put('copies',{id,email:current.email,kind:'prepared',driveId:data.fileId,revision:data.revision,blob:data.blob,name:String(data.name||'Rapport.pdf'),
            destination:{clientId:data.clientId,folderId:data.clientId,name:String(data.clientName||'Compagnie')},createdAt:Date.now(),status:'synced',uploadId:'',syncedUploadId:'',editVersion:0});
  ...
  window.addEventListener('online',()=>serial(async()=>{await refresh();await sync();}).catch(()=>{}));
