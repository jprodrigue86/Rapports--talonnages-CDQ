...
export function installTouchNavigation({container, surface, getViewer, now = Date.now}) {
  ...
  function startPan(t) {
    gesture = {mode:'pan', x:t.clientX, y:t.clientY,
      left:container.scrollLeft, top:container.scrollTop, moved:false};
  }
  function start(e) {
  ...
  function end(e) {
    ...
    if (e.type !== 'touchcancel' && e.touches.length === 1) startPan(e.touches[0]);
  }
  function click(e) {
...
export function installFormNavigation({surface, toolbar, previous, next, done, owner = document}) {
  ...
  done.addEventListener('click', () => {owner.activeElement?.blur(); active = null; update();});
