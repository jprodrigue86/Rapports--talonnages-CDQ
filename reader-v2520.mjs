import {installTouchNavigation,installFormNavigation} from './reader-interactions-v2520.mjs';
...
async function requestClose(){
  ...
  document.activeElement?.blur();await new Promise(r=>setTimeout(r,120));
...
async function open(data){
  ...
  const task=api.getDocument({data:new Uint8Array(await blob.arrayBuffer()),standardFontDataUrl:assets+'standard_fonts/',cMapUrl:assets+'cmaps/',cMapPacked:true,wasmUrl:assets+'wasm/',isEvalSupported:false,enableXfa:false});
  ...
  const fields=await doc.getFieldObjects(),actions=await doc.getJSActions();
  if(fields?.size||actions){const deadline=Date.now()+12000;while(!scripting.ready){if(Date.now()>deadline)throw Error('Les calculs du PDF n’ont pas pu démarrer. Fermez le document puis réessayez.');await new Promise(r=>setTimeout(r,25));}}
...
let drag=null;
$('container').addEventListener('pointerdown',e=>{if(e.pointerType==='mouse'&&e.button===1){e.preventDefault();drag={id:e.pointerId,x:e.clientX,y:e.clientY,left:$('container').scrollLeft,top:$('container').scrollTop};$('container').setPointerCapture(e.pointerId);}});
...
for(const type of ['pointerup','pointercancel'])$('container').addEventListener(type,()=>{drag=null});
...
try{
  ...
  viewer=new ui.PDFViewer({container:$('container'),viewer:$('viewer'),eventBus,linkService,scriptingManager:scripting,removePageBorders:true,annotationMode:api.AnnotationMode.ENABLE_FORMS,maxCanvasPixels:8388608});
  ...
  eventBus.on('annotationlayerrendered',()=>{if(readOnly)for(const field of $('viewer').querySelectorAll('input,textarea,select,button'))field.disabled=true;form.refresh();});
